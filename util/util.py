...
def intersectionAndUnionGPU(output, target, K, ignore_index=255, keys = None):
    # 'K' classes, output and target sizes are N or N * L or N * H * W, each value in range 0 to K - 1.
    ...
    output, target = mapSegmentOutputToPoints(output, target, keys)
...
def mapSegmentOutputToPoints(output, target, points):
    i = 0
    output_list = []
    target_list = []
    for segment in points.keys():
        for point in range(0, points[segment]):
            output_list.append(output[i])
            target_list.append(target[i])
        i+=1
    return torch.stack(output_list), torch.stack(target_list)
